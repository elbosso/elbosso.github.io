...
package de.elbosso.util.security;
...
/*
This code was derived from IETF RFC 1751. Please reproduce this note if possible.
 */
public class Rfc1751
{
	private final static org.slf4j.Logger CLASS_LOGGER =org.slf4j.LoggerFactory.getLogger(Rfc1751.class);
	public final static java.lang.String[] dictionary =
			{
	...
	private final static java.util.List<java.lang.String> dictionaryAsList = Arrays.asList(dictionary);
...
WENN SIE AUF DIE MOEGLICHKEIT EINES SOLCHEN SCHADENS HINGEWIESEN WORDEN SIND.
*/
	public static java.lang.String translate(byte[] bytes)
	{
		if (bytes == null)
			throw new java.lang.IllegalArgumentException("bytes must not be null!");
		if (bytes.length != 8)
			throw new java.lang.IllegalArgumentException("bytes must contain exactly 8 bytes!");
		byte[] input=Arrays.copyOf(bytes,bytes.length);
		java.lang.String rv = "";
		int p = 0;
		java.util.BitSet bits = java.util.BitSet.valueOf(input);
		...
		{
			input[i] = (byte) (Integer.reverse(input[i] << 24) & 0xff);
		}
		bits = java.util.BitSet.valueOf(input);
		...
		for (int i = 0; i < 6; ++i)
		{
			java.util.BitSet bs = bits.get(i * 11, (i + 1) * 11);
			java.util.BitSet reversed = new java.util.BitSet(11);
			for (int j = 0; j < 11; ++j)
			{
				reversed.set(10 - j, bs.get(j));
			}
			...
			/*for (int i = 0; i < dictionary.length; ++i)
			{
				...
				{
					de.elbosso.util.CLASS_LOGGER.debug(i + " " + java.lang.Integer.toHexString(i));
					break;
				}
			}*/
			if (rv.length() > 0)
	...
	private static java.lang.String standard(java.lang.String word)
	{
	...
	public static byte[] translate(java.lang.String encoded)
	{
		int i, p, l;
		byte[] b=new byte[9];

		if(encoded == null)
			throw new java.lang.IllegalArgumentException("parameter must not be null!");

		java.util.Arrays.fill(b,(byte)0);
		java.lang.String[] words=encoded.split(" ");
		if(words.length!=6)
			throw new java.lang.IllegalArgumentException("encoded string must contain exactly 6 words!");
		for(i=0,p=0;i<6;i++,p+=11){
			java.lang.String word=words[i];
			l = word.length();
			if(l > 4 || l < 1)
				throw new java.lang.IllegalArgumentException("encoded string may only contain words with 4 letters or less!");
			word = standard(word);
			int index = dictionaryAsList.indexOf(word);
			long cl;
			long cc;
			long cr;
			long y;
...
CLASS_LOGGER.debug("Parity value: "+b[b.length-1]);
		byte[] output=Arrays.copyOf(b,8);
		/* now check the parity of what we got */
		p = 0;
		java.util.BitSet bits = java.util.BitSet.valueOf(output);
		for (i = 0; i < 64; i += 2)
		{
			java.util.BitSet smallPart = bits.get(i, i + 2);
			long[] la = smallPart.toLongArray();
		...
		{
			output[i] = (byte) (Integer.reverse(output[i] << 24) & 0xff);
		}
		bits = java.util.BitSet.valueOf(output);
		java.util.BitSet parity = java.util.BitSet.valueOf(new long[]{p});
		byte parityByteComputed=0;
		if(parity.get(1))
			parityByteComputed|=(0x01<<7);
		if(parity.get(0))
			parityByteComputed|=(0x01<<6);
		CLASS_LOGGER.debug("Parity computed: "+(parity.get(1))+" "+(parity.get(0))+" "+parityByteComputed);
		if(parityByteComputed!=b[b.length-1])
			throw new de.elbosso.util.lang.CheckedExceptionWrapper("Parity check failed!");
		return Arrays.copyOf(b,8);
	}
